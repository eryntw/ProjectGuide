...
```{r birbase, echo = FALSE, fig.height = 10}
library(ggplot2)
library(dplyr)
...
# database
targets::tar_load(birdbase, store = tars$envBird$database$store)
df <- birdbase[, -c(1:16)]
cov <- knitr::knit_child("child/50_DbCovTp", quiet = TRUE)
cat(cov, sep = '\n')
```
...
## AVONET [@Tobias2022] {#avonet}

**AVONET** provides a globally standardised dataset of avian morphological traits, including measurements of the wing, bill, legs, and tail, alongside associated ecological and geographic information. The dataset integrates species-level data compiled from multiple sources, including BirdLife, eBird, and BirdTree.

The version of the database accompanying the original publication is more complete than that currently available through the `traitdata` R package. AVONET is particularly valuable for analyses of flight-related ecology, as it enables the use of traits linked to flight efficiency, dispersal capacity, and manoeuvrability, such as the Hand-Wing Index (HWI):
...
# database
df <- traitdata::avonet[, -c(1:4, 20:22)]
cov <- knitr::knit_child("child/50_DbCovTp", quiet = TRUE)
cat(cov, sep = '\n')

# metadata
meta <- readxl::read_excel(fs::path(here::here(), "..", "envBird", "database", 
...
* **Scope:** Global
* **Total number of species:** `r nrow(distinct(traitdata::avonet, Genus, Species))`
* **Total number of traits:** `r nrow(coverage)`
...
* The version distributed via the `traitdata` R package is less complete than the original published dataset

### Metadata

Metadata were obtained from the original publication. Note that some trait variables are only available in the original Excel dataset and are not included in the version distributed through the `traitdata` R package.
...
# database
attr <- readxl::read_excel(fs::path(here::here(), "..", "envBird", "database", 
                            "BirdLife_Australia", 
                            "Australian bird species attributes.xlsx"),
                   sheet = 1) %>% 
  janitor::clean_names(case = "snake")

tar_load(genlength, store = tars$envBird$database$store)

df <- attr %>% 
  inner_join(genlength %>% select(SpeciesName2024, GenerationLength), 
             by = c("scientific_name"="SpeciesName2024")) %>%
  dplyr::select(-1:-5)
...
## Australian Bird [@Garnett2015] {#ausbird}

This dataset provides a comprehensive and authoritative compilation of biological, ecological, conservation, and legal information for all Australian bird species and subspecies. It includes approximately 230 variables spanning taxonomy, population status, behaviour, breeding, mobility, and climate-related traits, and was developed with cross-referencing to HANZAB (Handbook of Australian, New Zealand & Antarctic Birds).
